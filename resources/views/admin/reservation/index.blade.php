                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($reservation->status == false)
                                                        ...
                                                        <form action="{{ route('reservation.status', $reservation->id) }}" method="POST" id="status-form-{{ $reservation->id }}" style="display: none;">
                                                            @csrf
                                                        </form>

                                                    @else
                                                        <button onclick="rejectReservation({{ $reservation->id  }})" type="button" class="btn btn-danger"><i class="material-icons">done</i></button>
                                                        <form action="{{ route('reservation.inverseStatus', $reservation->id) }}" method="POST" id="reject-form-{{ $reservation->id }}" style="display: none;">
                                                            @csrf
                                                        </form>
                                                    @endif
                                                    <button onclick="deleteReservation({{ $reservation->id  }})" type="button" class="btn btn-danger"><i class="material-icons">delete</i></button>
                                                    <form action="{{ route('reservation.destroy', $reservation->id) }}" method="POST" id="delete-form-{{ $reservation->id }}" style="display: none;">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </td>
